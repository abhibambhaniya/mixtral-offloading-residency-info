...
class MixtralExpertWrapper(nn.Module):
    """ A Wrapper for each expert in the MLP.
        The wrapper adds hooks to define how the weights are stored
    """
    def __init__(
        self,
        ...
        if isinstance(expert_module, MixtralBlockSparseTop2MLP):
            expert_module, self.storage = self.replace_layer_storage_full(expert_module, device)
        else:
            print(expert_module)
    ...
    @staticmethod
    def replace_layer_storage_full(
        layer: tp.Any,
        device: torch.device,
    ):
        state_dict = {
